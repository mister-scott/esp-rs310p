 ...
 * esp-rs310.cpp
 *
 * Implementation of esphome-rs310p
 *
    ...
    hw_serial_{hw_serial}
{
    ...
    this->traits_.set_visual_min_temperature(ESPRS310P_MIN_TEMPERATURE);
    this->traits_.set_visual_max_temperature(ESPRS310P_MAX_TEMPERATURE);
    this->traits_.set_visual_temperature_step(ESPRS310P_TEMPERATURE_STEP);
}

void ESPRS310P::check_logger_conflict_() {
#ifdef USE_LOGGER
    if (this->get_hw_serial_() == logger::global_logger->get_hw_serial()) {
        ESP_LOGW(TAG, "  You're using the same serial port for logging"
                " and the ESPRS310P component. Please disable"
...
void ESPRS310P::set_baud_rate(int baud) {
    this->baud_ = baud;
}
 ...
 */
output::OutputTraits ESPRS310P::traits() {
    return traits_;
}
 ...
 */
climate::ClimateTraits& ESPRS310P::config_traits() {
    return traits_;
}

/**
 * Implement control of a ESPRS310P.
 *
 * Maps HomeAssistant/ESPHome modes to Mitsubishi modes.
 */
void ESPRS310P::control(const climate::ClimateCall &call) {
...
void ESPRS310P::hpSettingsChanged() {
 ...
 */
void ESPRS310P::hpStatusChanged(heatpumpStatus currentStatus) {
...
void ESPRS310P::setup() {
    ...
    if (hp->connect(this->get_hw_serial_(), this->baud_, -1, -1)) {
        ...
        ESP_LOGCONFIG(
                ...
                " Marking ESPRS310P component as failed."
        );
        this->mark_failed();
    }
 ...
 **/
void ESPRS310P::save(float value, ESPPreferenceObject& storage) {
    uint8_t steps = (value - ESPRS310P_MIN_TEMPERATURE) / ESPRS310P_TEMPERATURE_STEP;
    storage.save(&steps);
}

optional<float> ESPRS310P::load(ESPPreferenceObject& storage) {
    uint8_t steps = 0;
    if (!storage.load(&steps)) {
        return {};
    }
    return ESPRS310P_MIN_TEMPERATURE + (steps * ESPRS310P_TEMPERATURE_STEP);
}

void ESPRS310P::dump_config() {
    this->banner();
...
void ESPRS310P::dump_state() {
    LOG_CLIMATE("", "ESPRS310P Climate", this);
    ESP_LOGI(TAG, "HELLO");
}